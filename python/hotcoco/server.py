...
def create_app(coco, image_dir: str | None = None, batch_size: int = 12, dt_coco=None) -> FastAPI:
    ...
    def _resolve_img_ids(categories: str | None, shuffle_seed: int | None) -> tuple[list[int], dict[int, int]]:
        """Filter by categories and optionally shuffle. Returns (img_ids, id_to_pos)."""
        key = (categories, shuffle_seed)
        ...
        if not categories:
            ...
            img_ids = list(coco.get_img_ids(cat_ids=cat_ids)) if cat_ids else list(all_img_ids)
    ...
    def _build_query(categories, shuffle_seed, min_score) -> str:
        parts = []
        if categories:
    ...
    async def gallery(
        ...
        img_ids, _ = _resolve_img_ids(categories, shuffle_seed)
        ...
        filter_query = _build_query(categories, shuffle_seed, min_score)
    ...
    async def detail(
        ...
        img_ids, id_to_pos = _resolve_img_ids(categories, shuffle_seed)
        nav = _get_nav(img_ids, id_to_pos, image_id)

        nav_query = _build_query(categories, shuffle_seed, min_score)
