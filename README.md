...
- **Tracking evaluation** — HOTA, CLEAR/MOTA, and Identity/IDF1 metrics for multi-object tracking. Uses TAO-style COCO JSON with `video_id`, `track_id`, and `frame_index` fields. Includes MOTChallenge format import. See [tracking evaluation](https://derekallman.github.io/hotcoco/guide/tracking/).
- **Format conversion** — COCO ↔ YOLO, COCO ↔ Pascal VOC, COCO ↔ CVAT, and COCO ↔ MOT from Python or the CLI; COCO ↔ DOTA from Python. See [format conversion](https://derekallman.github.io/hotcoco/guide/conversion/).
